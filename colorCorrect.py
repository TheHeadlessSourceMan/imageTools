...
def perPixel(fn,image,clamp=True):
    ...
    if not isinstance(fn,np.lib.function_base.vectorize):
...
def temperatureToRgb(temperature):
    ...
    NOTES:
        ...
        But we use this for a closer approximation:
            (still not acceptable for scientific applications)
...
def toneCombine(image,tones,toneAmounts=1.0):
    ...
    if isinstance(toneAmounts,(int,float)):
...
def tritoneTint(image,tintColors,tintAmounts=1.0,atPoints=None,absolute=False):
    ...
    if isinstance(tintAmounts,(int,float)):
...
def photoFilter(image,photoFilter,density=1.0,preserveLuminosity=True):
    """
...
def equalize(image,amount=1.0,colorChannels=False):
    ...
    if len(image.shape)<3:
        ...
        cdf=image_histogram.cumsum() # cdf="Cumulative Distribution Function"
        cdf=255*cdf/cdf[-1] # normalize
        # use linear interpolation of cdf to find new pixel values
        imageNew=np.interp(imageNew,bins[:-1],cdf)*amount+imageNew/amount
        image=imageNew.reshape(image.shape),cdf
    return image
...
def colorize(image,color,keepSaturation=0.0):
    ...
    :param keepSaturation: how much of the original saturation to keep
        (verses take from the given color)
...
def cmdline(args):
    ...
    if printHelp:
        ...
        print('   --adjustTemperature=t ......... adjust to the given color')
